/* =============================================================================
 * coreview.js — a drop-in visual review & annotation layer for any website.
 * Open-source edition · local-only · zero backend.
 *
 * Load with a single <script> tag:
 *   <script src="coreview.js" defer></script>
 *
 ...
 * …or via `window.CoReviewConfig = { project, page, … }` before the script.
 *
 * Tools: text highlight, rectangle, circle, pin, freehand ink, section
 * notes and screenshot crop — each carries a threaded comment.
 *
...
(function () {
  "use strict";
  if (window.__COREVIEW_LOADED__) return;
  window.__COREVIEW_LOADED__ = true;

  var VERSION = "1.1.0";

  // --------------------------------------------------------------------------
  // SCREENSHOT — html2canvas lazy loader + capture helper
  // Added for SCORM use case: captures the visible page at comment-creation time
  // so reviewers can see exactly which slide/state was being annotated.
  // --------------------------------------------------------------------------
  var _h2cReady = false, _h2cCallbacks = [];
  function loadHtml2Canvas(cb) {
    // If html2canvas is already on the page (pre-loaded by the host, e.g. SCORM
    // index.html), mark ready immediately and skip the CDN injection entirely.
    if (window.html2canvas) { _h2cReady = true; cb(window.html2canvas); return; }
    if (_h2cReady) { cb(window.html2canvas); return; }
    _h2cCallbacks.push(cb);
    if (document.getElementById("__an_h2c_script")) return;
    var s = document.createElement("script");
    s.id = "__an_h2c_script";
    ...
    s.onerror = function () {
      _h2cCallbacks.forEach(function (fn) { fn(null); });
      _h2cCallbacks = [];
    };
    document.head.appendChild(s);
  }

  // Capture an arbitrary viewport rectangle chosen by the crop tool.
  // cropRect: { x, y, w, h } all in viewport (CSS pixel) coordinates.
  function captureCropRegion(cropRect, cb) {
    loadHtml2Canvas(function (h2c) {
      if (!h2c) { cb(null); return; }

      var cropX = Math.round(Math.max(0, cropRect.x));
      var cropY = Math.round(Math.max(0, cropRect.y));
      var cropW = Math.round(Math.min(window.innerWidth  - cropX, cropRect.w));
      var cropH = Math.round(Math.min(window.innerHeight - cropY, cropRect.h));
      if (cropW < 4 || cropH < 4) { cb(null); return; }

      // Hide all annotate UI before capturing
      var IDS = ["__an_root","__an_bar","__an_panel","__an_compose","__an_launch",
                 "__an_overlay","__an_pins","__an_plus","__an_hint","__an_toasts",
                 "__an_namewrap","__an_sharewrap","__an_h2c_script"];
      var hidden = IDS.map(function (id) { return document.getElementById(id); }).filter(Boolean);
      hidden.forEach(function (n) { n.style.visibility = "hidden"; });

      // Base html2canvas options — used for both attempts
      var baseOpts = {
        allowTaint: true,
        scale: 1,
      ...
      function cropAndEncode(fullCanvas) {
        ...
        if (out.width > maxW) {
          ...
          scaled.width  = maxW;
          scaled.height = Math.max(1, Math.round(out.height * (maxW / out.width)));
          scaled.getContext("2d").drawImage(out, 0, 0, scaled.width, scaled.height);
          out = scaled;
        }
        // toDataURL throws SecurityError if canvas is tainted — return null so
        // caller can try the fallback pass
        try { return out.toDataURL("image/jpeg", 0.85); } catch (e) { return null; }
      }

      function restore() { hidden.forEach(function (n) { n.style.visibility = ""; }); }

      // Pass 1 — useCORS:true (works when assets are on same origin / CORS-enabled)
      h2c(document.body, Object.assign({}, baseOpts, { useCORS: true }))
        .then(function (fullCanvas) {
          var dataUrl = cropAndEncode(fullCanvas);
          if (dataUrl) { restore(); cb(dataUrl); return; }

          // Pass 2 — foreignObjectRendering:true avoids canvas taint entirely by
          // serialising the DOM to SVG. Works with blob: URLs (SCORM bundles).
          h2c(document.body, Object.assign({}, baseOpts, {
            useCORS: false,
            foreignObjectRendering: true,
          })).then(function (fullCanvas2) {
            restore();
            var dataUrl2 = cropAndEncode(fullCanvas2);
            cb(dataUrl2 || null);
          }).catch(function () { restore(); cb(null); });
        })
        .catch(function () {
          // Pass 1 threw entirely — jump straight to foreignObjectRendering
          h2c(document.body, Object.assign({}, baseOpts, {
            useCORS: false,
            foreignObjectRendering: true,
          })).then(function (fullCanvas2) {
  ...
  var SCRIPT = document.currentScript ||
    document.querySelector('script[src*="coreview"]') ||
    document.querySelector('script[src*="annotate"]');
  var scriptData = (SCRIPT && SCRIPT.dataset) || {};   // data-* attributes
  var globalConfig = window.CoReviewConfig || window.AnnotateConfig || {};  // config object
  var CFG = {
  ...
  var state = {
    tool: "cursor", // cursor | highlight | rect | circle | pin | pen | crop
    color: store.get("an-color") || COLORS[0].hex,
    author: store.get("an-author") || "",
    // role removed — all users have the same access
    ...
    panelOpen: false,
    activeId: null,
    filter: "open", // open | in-progress | fixed | verified | all
    query: "",
  ...
  function createComment(draft) {
    ...
    var c = {
      ...
      priority: draft.priority || "medium", // low | medium | high | critical
      status: "open",                        // open | in-progress | fixed | verified | wont-fix | reopened
      ...
      screenshotViewport: draft.screenshotViewport || null,
      resolved: false,
      replies: [],
      createdAt: now,
      updatedAt: now,
  ...
  function patchComment(id, changes) {
    ...
    if (typeof changes.status === "string") {
      ...
      // keep legacy resolved flag in sync
      c.resolved = (changes.status === "verified");
    }
  ...
  #__an_crop_backdrop {
    position: fixed; inset: 0; z-index: 2147483250;
    background: rgba(0,0,0,.38); pointer-events: none;
  }
  #__an_crop_sel {
    position: fixed; z-index: 2147483260; pointer-events: none;
    border: 2px dashed #fff;
    box-shadow: 0 0 0 9999px rgba(0,0,0,.38), 0 0 0 1px rgba(255,255,255,.25) inset;
    border-radius: 3px;
    animation: an-crop-dash 0.6s linear infinite;
  }
  @keyframes an-crop-dash {
    from { border-dash-offset: 0; }
    to   { border-dash-offset: 20px; }
  }
  #__an_crop_sel .an-crop-dims {
    position: absolute; bottom: calc(100% + 6px); left: 0;
    background: #1c1c24; color: #f2f2f6;
    font: 600 11px/1 var(--an-font); padding: 4px 8px; border-radius: 6px;
    white-space: nowrap; pointer-events: none;
  }
  body.an-cropping { cursor: crosshair !important; touch-action: none; user-select: none; -webkit-user-select: none; }
  body.an-cropping ::selection { background: transparent; }

  /* ---- priority + status badges ------------------------------------------- */
  .an-pri-badge {
    display: inline-flex; align-items: center; gap: 3px;
    font: 600 10px/1 var(--an-font); padding: 2px 7px; border-radius: 20px;
    flex: none;
  }
  .an-pri-low      { background: rgba(16,185,129,.12); color: #10b981; }
  .an-pri-medium   { background: rgba(99,102,241,.12); color: #6366f1; }
  .an-pri-high     { background: rgba(245,158,11,.15); color: #f59e0b; }
  .an-pri-critical { background: rgba(239,68,68,.13);  color: #ef4444; }

  .an-status-badge {
    display: inline-flex; align-items: center; gap: 3px;
    font: 600 10px/1 var(--an-font); padding: 2px 8px; border-radius: 20px;
    flex: none;
  }
  .an-st-open       { background: rgba(99,102,241,.12); color: #6366f1; }
  .an-st-in-progress{ background: rgba(245,158,11,.15); color: #f59e0b; }
  .an-st-fixed      { background: rgba(16,185,129,.13); color: #10b981; }
  .an-st-verified   { background: rgba(16,185,129,.22); color: #059669; }
  .an-st-reopened   { background: rgba(239,68,68,.13);  color: #ef4444; }

  /* ---- status action row on cards ---------------------------------------- */
  .an-status-row { display:flex; gap:5px; flex-wrap:wrap; margin-top:8px; }
  .an-status-btn {
    border: 1px solid var(--an-border); background: var(--an-surface);
    border-radius: 8px; padding: 4px 10px; font: 500 11.5px var(--an-font);
    cursor: pointer; color: var(--an-muted);
    display: inline-flex; align-items: center; gap: 4px;
    transition: all .15s;
  }
  .an-status-btn:hover { background: var(--an-surface-2); color: var(--an-fg); border-color: var(--an-border-strong); }
  .an-status-btn.an-active-status { border-color: currentColor; opacity: 1; }
  .an-status-btn.an-st-action-inprogress:hover { color: #f59e0b; border-color: #f59e0b; background: rgba(245,158,11,.08); }
  .an-status-btn.an-st-action-fixed:hover      { color: #10b981; border-color: #10b981; background: rgba(16,185,129,.08); }
  .an-status-btn.an-st-action-verified:hover   { color: #059669; border-color: #059669; background: rgba(16,185,129,.12); }
  .an-status-btn.an-st-action-reopen:hover     { color: #ef4444; border-color: #ef4444; background: rgba(239,68,68,.08); }

  /* ---- manager summary bar ----------------------------------------------- */
  #__an_summary {
    display: none; flex-wrap: wrap; gap: 6px; align-items: center;
    padding: 8px 18px 4px; font: 500 12px var(--an-font);
  }
  #__an_summary.an-show { display: flex; }
  .an-sum-item {
  ...
  .an-sum-item:hover { border-color: var(--an-border-strong); background: var(--an-surface); }
  .an-sum-item.an-on { border-color: currentColor; }
  .an-sum-dot { width: 7px; height: 7px; border-radius: 50%; flex: none; }
  .an-sum-label { color: var(--an-muted); }
  .an-sum-count { font-weight: 700; color: var(--an-fg); }

  /* ---- priority selector in composer ------------------------------------- */
  .an-pri-row { display:flex; gap:5px; margin-bottom:8px; flex-wrap:wrap; }
  .an-pri-opt {
    border: 1px solid var(--an-border); background: var(--an-surface-2);
    border-radius: 8px; padding: 4px 10px; font: 500 11.5px var(--an-font);
    cursor: pointer; color: var(--an-muted); transition: all .15s;
  }
  .an-pri-opt:hover { border-color: var(--an-border-strong); color: var(--an-fg); }
  .an-pri-opt.an-on { border-color: currentColor; background: var(--an-surface); }
  .an-pri-opt.an-pri-low.an-on      { color: #10b981; }
  .an-pri-opt.an-pri-medium.an-on   { color: #6366f1; }
  .an-pri-opt.an-pri-high.an-on     { color: #f59e0b; }
  .an-pri-opt.an-pri-critical.an-on { color: #ef4444; }

  /* ---- screenshot thumbnail (shown inside comment card) ---- */
  .an-thumb-wrap { margin: 8px 0 4px; }
  .an-thumb {
    display: block; width: 100%; max-height: 120px;
    object-fit: cover; object-position: top left;
    border-radius: 8px; border: 1px solid var(--an-border);
    cursor: zoom-in; transition: opacity .15s, box-shadow .15s;
  }
  .an-thumb:hover { opacity: .85; box-shadow: var(--an-shadow-sm); }
  .an-thumb-lbl { font-size: 10px; color: var(--an-muted); margin-top: 3px; }

  /* ---- screenshot lightbox modal ---- */
  #__an_scmodal {
    position: fixed; inset: 0; z-index: 2147483600;
    background: rgba(10,10,18,.85); backdrop-filter: blur(6px);
    display: flex; align-items: center; justify-content: center;
    font-family: var(--an-font); animation: an-fade-in .18s ease;
  }
  @keyframes an-fade-in { from { opacity: 0 } to { opacity: 1 } }
  #__an_scmodal .an-scbox {
    position: relative; max-width: min(960px, 96vw); max-height: 92vh;
    background: #111; border-radius: 14px; overflow: hidden;
    box-shadow: var(--an-shadow-lg);
  }
  #__an_scmodal img { display: block; max-width: 100%; max-height: 88vh; }
  #__an_scmodal .an-scclose {
  ...
  #__an_scmodal .an-scclose:hover { background: rgba(255,255,255,.32); }
  #__an_scmodal .an-sclbl {
  ...
  var ICONS = {
    ...
    crop: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2v14a2 2 0 0 0 2 2h14"/><path d="M18 22V8a2 2 0 0 0-2-2H2"/><rect x="8" y="8" width="8" height="8" rx="1" stroke-dasharray="3 2"/></svg>',
  };
  ...
  function openComposer(x, y, draft) {
    ...
    pendingDraft = draft;
    var label = { highlight: "Highlight", shape: draft.geom && draft.geom.kind === "circle" ? "Circle" : "Rectangle", pin: "Pin", pen: "Sketch", block: "Section", screenshot: "Screenshot" }[draft.type] || "Note";
    composer.innerHTML = "";
    ...
    // Show a preview of the cropped screenshot inside the composer
    if (draft.type === "screenshot" && draft.screenshot) {
      var prevWrap = el("div", { style: "margin-bottom:9px;border-radius:8px;overflow:hidden;border:1px solid var(--an-border);" });
      var prevImg = el("img", { src: draft.screenshot, alt: "Captured region", style: "display:block;width:100%;max-height:130px;object-fit:cover;object-position:top left;" });
      prevWrap.appendChild(prevImg);
      composer.appendChild(prevWrap);
    }
    ...
    var priRow = el("div", { class: "an-pri-row" });
    [["low","Low","an-pri-low"], ["medium","Medium","an-pri-medium"],
     ["high","High","an-pri-high"], ["critical","Critical","an-pri-critical"]
    ].forEach(function (p) {
      var opt = el("button", { class: "an-pri-opt " + p[2] + (p[0] === "medium" ? " an-on" : ""), text: p[1] });
      opt.addEventListener("click", function (e) {
        e.stopPropagation();
        selectedPriority = p[0];
        priRow.querySelectorAll(".an-pri-opt").forEach(function (x) { x.classList.remove("an-on"); });
        opt.classList.add("an-on");
      });
      priRow.appendChild(opt);
    });
    composer.appendChild(priRow);
  ...
  function askName(onDone) {
    ...
    var kids = [
      el("h3", { class: "an-nt", text: "Who are you?" }),
      el("p", { class: "an-nd", text: "Your name appears on every comment so the team knows who raised what. Saved on this device." }),
    ];
    if (state.note) {
    ...
    kids.push(
      el("p", { style: "font:600 11px var(--an-font);text-transform:uppercase;letter-spacing:.05em;color:var(--an-muted);margin-bottom:8px;margin-top:4px;", text: "Your name" }),
      input,
      btn
    );
  ...
  function cancelDraft() {
    pendingDraft = null;
    clearTemp();
    // Also clean up any in-progress crop drag
    if (cropDraw) {
      cropDraw.backdrop.remove();
      cropDraw.sel.remove();
      document.body.classList.remove("an-cropping");
      cropDraw = null;
    }
  ...
  function commitDraft(draft) {
    // Hide composer immediately so it doesn't appear in the screenshot
    composer.classList.remove("an-show");
    clearTemp();
    // Crop tool pre-attaches the screenshot — save directly, no capture needed
    if (draft.type === "screenshot" && draft.screenshot) {
      ...
      setTool("cursor");
      pendingDraft = null;
      toast("Screenshot comment saved", { kind: "success", duration: 2500 });
      return;
    }
    // All other tools (rect, circle, pen, pin, highlight, block) save without screenshot
    draft.screenshot = null;
    draft.screenshotViewport = null;
    var c = createComment(draft);
    state.comments.push(c);
    state.activeId = c.id;
    renderAll();
  ...
  var drawing = null;
  var cropDraw = null; // tracks active crop drag
  function onDown(e) {
    if (!state.enabled) return;
    ...
    if (t === "crop") {
      e.preventDefault();
      // Build the dimming backdrop + selection box elements
      var backdrop = el("div", { id: "__an_crop_backdrop" });
      var sel = el("div", { id: "__an_crop_sel" });
      var dims = el("div", { class: "an-crop-dims", text: "0 × 0" });
      sel.appendChild(dims);
      document.body.appendChild(backdrop);
      document.body.appendChild(sel);
      document.body.classList.add("an-cropping");
      cropDraw = { startX: e.clientX, startY: e.clientY, backdrop: backdrop, sel: sel, dims: dims };
      if (e.pointerId != null && document.body.setPointerCapture)
        try { document.body.setPointerCapture(e.pointerId); } catch (ex) {}
      return;
    }
  ...
  function onMove(e) {
    if (cropDraw) {
      e.preventDefault();
      var cd = cropDraw;
      var x = Math.min(cd.startX, e.clientX), y = Math.min(cd.startY, e.clientY);
      var w = Math.abs(e.clientX - cd.startX), h = Math.abs(e.clientY - cd.startY);
      cd.sel.style.left   = x + "px";
      cd.sel.style.top    = y + "px";
      cd.sel.style.width  = w + "px";
      cd.sel.style.height = h + "px";
      cd.dims.textContent = Math.round(w) + " × " + Math.round(h);
      return;
    }
    if (!drawing) return;
    e.preventDefault();
  ...
  function onUp(e) {
    if (cropDraw) {
      var cd = cropDraw; cropDraw = null;
      var x = Math.min(cd.startX, e.clientX), y = Math.min(cd.startY, e.clientY);
      var w = Math.abs(e.clientX - cd.startX), h = Math.abs(e.clientY - cd.startY);
      // Clean up overlay elements + cursor
      cd.backdrop.remove();
      cd.sel.remove();
      document.body.classList.remove("an-cropping");
      // Ignore tiny drags (accidental clicks)
      if (w < 10 || h < 10) { justCancelledDraw = true; setTool("cursor"); return; }
      // Show a "capturing…" toast, then take the screenshot
      var captureToast = toast("Capturing screenshot…", { kind: "info", duration: 8000 });
      captureCropRegion({ x: x, y: y, w: w, h: h }, function (dataUrl) {
        captureToast.dismiss();
        if (!dataUrl) {
          toast("Screenshot failed — try again", { kind: "error" });
          setTool("cursor");
          return;
        }
        // Open the composer with the screenshot already attached
        openComposer(
          Math.min(e.clientX + 6, window.innerWidth - 320),
          Math.min(e.clientY + 6, window.innerHeight - 240),
          {
            type: "screenshot",
            color: state.color,
            screenshot: dataUrl,
            screenshotViewport: { vw: window.innerWidth, vh: window.innerHeight },
            geom: null,
          }
        );
        setTool("cursor");
  ...
  function buildUI() {
    ...
    [["V", "Browse"], ["H", "Highlight"], ["R", "Rectangle"], ["C", "Circle"],
     ["D", "Freehand"], ["P", "Pin"], ["S", "Screenshot region"], ["A", "Comments panel"], ["O", "Show / hide tools"],
    ...
    [["open","Open"],["in-progress","In Progress"],["fixed","Fixed"],
     ["verified","Verified"],["all","All"]].forEach(function (f) {
    ...
    var summaryEl = el("div", { id: "__an_summary" });
    ...
    document.addEventListener("keydown", function (e) {
      ...
      if (!state.enabled) return;
      var map = { v: "cursor", h: "highlight", r: "rect", c: "circle", d: "pen", p: "pin", s: "crop" };
  ...
  function setTool(t) {
    ...
    var drawingTool = t === "rect" || t === "circle" || t === "pen" || t === "pin" || t === "crop";
    ...
    var hints = { highlight: "Select any text to highlight & comment",
      ...
      crop: "Drag to capture a screenshot region" };
  ...
  var TYPE_LABEL = { highlight: "Highlight", shape: "Shape", pin: "Pin", pen: "Sketch", note: "Note", block: "Section", screenshot: "Screenshot" };

  var PRI_META = {
    low:      { cls: "an-pri-low",      label: "Low",      dot: "#10b981" },
    medium:   { cls: "an-pri-medium",   label: "Medium",   dot: "#6366f1" },
    high:     { cls: "an-pri-high",     label: "High",     dot: "#f59e0b" },
    critical: { cls: "an-pri-critical", label: "Critical", dot: "#ef4444" },
  };
  var STATUS_META = {
    "open":        { cls: "an-st-open",        label: "Open",        dot: "#6366f1" },
    "in-progress": { cls: "an-st-in-progress", label: "In Progress", dot: "#f59e0b" },
    "fixed":       { cls: "an-st-fixed",       label: "Fixed",       dot: "#10b981" },
    "verified":    { cls: "an-st-verified",    label: "Verified",    dot: "#059669" },
    "reopened":    { cls: "an-st-reopened",    label: "Reopened",    dot: "#ef4444" },
  };
  function priBadge(pri) {
    var m = PRI_META[pri || "medium"]; if (!m) return null;
    var dot = el("span", { class: "an-sum-dot" }); dot.style.background = m.dot;
    return el("span", { class: "an-pri-badge " + m.cls }, [dot, document.createTextNode(m.label)]);
  }
  function statusBadge(status) {
    var m = STATUS_META[status || "open"]; if (!m) return null;
    var dot = el("span", { class: "an-sum-dot" }); dot.style.background = m.dot;
    return el("span", { class: "an-status-badge " + m.cls }, [dot, document.createTextNode(m.label)]);
  }
  ...
  function priBadge(pri) {
    var m = PRI_META[pri || "medium"];
    if (!m) return null;
    var dot = el("span", { class: "an-sum-dot" });
    dot.style.background = m.dot;
    return el("span", { class: "an-pri-badge " + m.cls }, [dot, document.createTextNode(m.label)]);
  }
  function statusBadge(status) {
    var m = STATUS_META[status || "open"];
    if (!m) return null;
    var dot = el("span", { class: "an-sum-dot" });
    dot.style.background = m.dot;
    return el("span", { class: "an-status-badge " + m.cls }, [dot, document.createTextNode(m.label)]);
  ...
  function visibleComments() {
    return state.comments.filter(function (c) {
      var curStatus = c.status || (c.resolved ? "verified" : "open");
      if (state.filter === "open" && curStatus !== "open" && curStatus !== "reopened") return false;
      if (state.filter === "in-progress" && curStatus !== "in-progress") return false;
      if (state.filter === "fixed" && curStatus !== "fixed") return false;
      if (state.filter === "verified" && curStatus !== "verified") return false;
  ...
  // --------------------------------------------------------------------------
  // SCREENSHOT MODAL — opens a lightbox showing the full screenshot
  // --------------------------------------------------------------------------
  function openScreenshotModal(c) {
    if (!c.screenshot) return;
    if (document.getElementById("__an_scmodal")) return;
    var wrap = el("div", { id: "__an_scmodal" });
    var box  = el("div", { class: "an-scbox" });
    var closeBtn = el("button", { class: "an-scclose", html: "&times;", "aria-label": "Close screenshot" });
    closeBtn.addEventListener("click", function () { wrap.remove(); });
    wrap.addEventListener("click", function (e) { if (e.target === wrap) wrap.remove(); });
    document.addEventListener("keydown", function escHandler(e) {
      if (e.key === "Escape") { wrap.remove(); document.removeEventListener("keydown", escHandler); }
    });
    var img = el("img", { src: c.screenshot, alt: "Page screenshot at time of comment" });
    box.appendChild(img);
    box.appendChild(closeBtn);
    if (c.text) box.appendChild(el("div", { class: "an-sclbl", text: "\u201c" + c.text + "\u201d" }));
    wrap.appendChild(box);
    document.body.appendChild(wrap);
  }

  function renderSummary() {
    var sumEl = document.getElementById("__an_summary");
    if (!sumEl) return;
    sumEl.innerHTML = "";
    var all = state.comments;
    if (!all.length) { sumEl.classList.remove("an-show"); return; }

    // Count by status
    var counts = { "open": 0, "in-progress": 0, "fixed": 0, "verified": 0, "reopened": 0 };
    all.forEach(function (c) {
      var s = c.status || (c.resolved ? "verified" : "open");
      if (counts[s] !== undefined) counts[s]++;
    });

    var shown = [
      ["open",        counts["open"] + counts["reopened"], STATUS_META["open"].dot],
      ["in-progress", counts["in-progress"],               STATUS_META["in-progress"].dot],
      ["fixed",       counts["fixed"],                     STATUS_META["fixed"].dot],
      ["verified",    counts["verified"],                  STATUS_META["verified"].dot],
    ].filter(function (r) { return r[1] > 0; });

    if (!shown.length) { sumEl.classList.remove("an-show"); return; }

    shown.forEach(function (r) {
      var filterKey = r[0], count = r[1], color = r[2];
      var dot = el("span", { class: "an-sum-dot" }); dot.style.background = color;
      var label = STATUS_META[filterKey] ? STATUS_META[filterKey].label : filterKey;
      var item = el("div", {
        class: "an-sum-item" + (state.filter === filterKey ? " an-on" : ""),
        style: state.filter === filterKey ? "border-color:" + color : "",
      }, [
        dot,
        el("span", { class: "an-sum-count", text: String(count) }),
        el("span", { class: "an-sum-label", text: " " + label }),
      ]);
      item.addEventListener("click", function () {
        state.filter = filterKey;
        document.querySelectorAll("#__an_panel .an-chip").forEach(function (x) {
          x.classList.toggle("an-on", x.getAttribute("data-f") === filterKey);
        });
        renderAll(); renderPanel();
      });
      sumEl.appendChild(item);
    });
    sumEl.classList.add("an-show");
  }
  ...
  function renderPanel() {
    ...
    list.forEach(function (c) {
      ...
      var meta = el("div", { class: "an-cmeta" }, [
        ...
        priBadge(c.priority),
        statusBadge(c.status || (c.resolved ? "verified" : "open")),
      ...
      // Screenshot thumbnail — shown if a screenshot was captured when the comment was created
      if (c.screenshot) {
        var thumbWrap = el("div", { class: "an-thumb-wrap" });
        var thumb = el("img", { class: "an-thumb", src: c.screenshot, alt: "Screenshot", title: "Click to enlarge" });
        thumb.addEventListener("click", function (e) { e.stopPropagation(); openScreenshotModal(c); });
        thumbWrap.appendChild(thumb);
        thumbWrap.appendChild(el("div", { class: "an-thumb-lbl", text: "\uD83D\uDCF7 Screenshot at time of comment" }));
      ...
      // ---- status action row (role-aware) ---------------------------------
      // What actions are shown depends on the current status and the viewer's role.
      var curStatus = c.status || (c.resolved ? "verified" : "open");
      var statusRow = el("div", { class: "an-status-row" });

      function makeStatusBtn(label, newStatus, extraCls) {
        var b = el("button", { class: "an-status-btn " + (extraCls || "") });
        b.textContent = label;
        b.addEventListener("click", function (e) {
          e.stopPropagation();
          var updated = patchComment(c.id, { status: newStatus });
          if (updated) { mergeComment(updated); renderAll(); renderPanel(); }
        });
        return b;
      }

      // Universal actions — visible to everyone regardless of who they are
      if (curStatus === "open" || curStatus === "reopened") {
        statusRow.appendChild(makeStatusBtn("▶ In Progress", "in-progress", "an-st-action-inprogress"));
        statusRow.appendChild(makeStatusBtn("✓ Mark Fixed",  "fixed",       "an-st-action-fixed"));
      }
      if (curStatus === "in-progress") {
        statusRow.appendChild(makeStatusBtn("✓ Mark Fixed", "fixed", "an-st-action-fixed"));
      }
      if (curStatus === "fixed") {
        statusRow.appendChild(makeStatusBtn("✓ Verified", "verified",  "an-st-action-verified"));
        statusRow.appendChild(makeStatusBtn("↩ Reopen",   "reopened",  "an-st-action-reopen"));
      }
      ...
      var act = el("div", { class: "an-cact" }, [
        ...
        el("button", { class: "an-mini", html: ICONS.edit + "<span>Edit</span>", onclick: function (e) {
          e.stopPropagation();
          eta.value = c.text || "";
  ...
  // Backward-compat alias — old code using window.Annotate still works
  window.Annotate = window.CoReview;
